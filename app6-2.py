        ...
        if results:
            for i, page in enumerate(results, 1):
                with st.container():
                    ...
                    with col1:
                        st.caption(f"👤 {page.get('author', '不明')}")
                    with col2:
                        st.caption(f"📊 {page.get('word_count', 0)}語")
                    with col3:
                        st.caption(f"📁 {page.get('category', '未分類')}")
                    with col4:
...
with tab_list:
    ...
    if not pages:
        st.info("登録されているページがありません。クローラータブから追加してください。")
    else:
        for page in pages:
            with st.expander(f"📄 {page['title']}"):
                st.markdown(f"**URL:** {page['url']}")
                st.markdown(f"**説明:** {page.get('description', '(なし)')}")

                col1, col2, col3 = st.columns(3)
                with col1:
                    st.caption(f"語数: {page.get('word_count', 0)}")
                with col2:
                    st.caption(f"作成者: {page.get('author', '不明')}")
                with col3:
                    st.caption(f"カテゴリ: {page.get('category', '未分類')}")
